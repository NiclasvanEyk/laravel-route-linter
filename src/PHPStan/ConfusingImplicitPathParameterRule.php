...
readonly final class ConfusingImplicitPathParameterRule implements Rule
{
    /**
     * @var Violation[]
     */
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (!($method instanceof ExtendedMethodReflection)) return [];

        $errors = [];
        foreach ($this->violations as $violation) {
            $handler = $violation->route->handler;
            if ($handler->getDeclaringClass()->getName() !== $method->getDeclaringClass()->getName()) continue;
            if ($handler->getName() !== $method->getName()) continue;
